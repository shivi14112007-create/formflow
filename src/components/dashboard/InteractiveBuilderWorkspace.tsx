...
export const InteractiveBuilderWorkspace: React.FC<InteractiveBuilderWorkspaceProps> = ({
  ...
  const [formDescription, setFormDescription] = React.useState('Help us improve FormFlow with your instant serverless feedback.');
  ...
  const handleLoadSampleTemplate = () => {
    const sampleFields = [
      { id: 'field-1', label: 'Full Name', type: 'text', required: true, placeholder: 'Enter your first and last name' },
      { id: 'field-2', label: 'Work Email Address', type: 'email', required: true, placeholder: 'name@company.com' },
      { id: 'field-3', label: 'Self Assessment', type: 'select', required: false, options: ['Junior Architect', 'Staff Lead', 'Principal DevOps', 'Independent Advisor'] },
    ];
    setFields(sampleFields);
    setSelectedFieldId('field-1');
    setFormTitle('Product Survey & Assessment');
    setFormDescription('Configure your serverless product research data points in under 60 seconds.');
  ...
  const [previewFormValues, setPreviewFormValues] = React.useState<Record<string, any>>({});
  const [previewErrors, setPreviewErrors] = React.useState<Record<string, string>>({});
  const [previewTheme, setPreviewTheme] = React.useState<'indigo' | 'emerald' | 'amber' | 'rose' | 'slate'>('indigo');
  const [previewBreakpoint, setPreviewBreakpoint] = React.useState<'mobile' | 'tablet' | 'desktop'>('desktop');
  ...
  const [formSubmissionSuccessMessage, setFormSubmissionSuccessMessage] = React.useState<string | null>(null);
  ...
  const [recentEntries, setRecentEntries] = React.useState<Array<{
    ...
    { id: 'Ingest-742910', email: 'justin@vercel.com', browser: 'Chrome', time: '2 mins ago', submittedData: { 'Full Name': 'Justin', 'Work Email Address': 'justin@vercel.com', 'How did you hear about FormFlow?': 'Product Hunt' } },
    { id: 'Ingest-391820', email: 'aws-ops@amazon.co.uk', browser: 'Firefox', time: '12 mins ago', submittedData: { 'Company Legal Name': 'Amazon UK Ops', 'Number of Seats': '250', 'Cloud Hosting Preference': 'AWS Us-East-1' } },
    { id: 'Ingest-821035', email: 'framer-dev@framer.de', browser: 'Safari', time: '1 hr ago', submittedData: { 'Full Name': 'Framer Dev', 'Work Email Address': 'framer-dev@framer.de' } },
    { id: 'Ingest-140284', email: 'shivi14112007@gmail.com', browser: 'Edge', time: '3 hrs ago', submittedData: { 'Full Name': 'Shivi', 'Work Email Address': 'shivi14112007@gmail.com' } },
  ]);
  ...
  const handleAddField = (type: FormField['type']) => {
    ...
    const newField: FormField = {
      ...
      required: false,
      placeholder: type === 'text' ? 'Write your response...' : type === 'email' ? 'you@example.com' : undefined,
  ...
  const templates: FormTemplate[] = [
    {
      ...
      description: 'Collect details from high-tier enterprise clients with automated webhook integrations.',
      ...
      fields: [
        ...
        { id: 'on-3', type: 'select', label: 'Cloud Hosting Preference', required: true, options: ['AWS Us-East-1', 'GCP Iowa', 'SaaS Multi-tenant'] },
      ]
    },
    {
      id: 'template-2',
      title: 'Lambda Endpoint Bug Report',
      description: 'Streamline developer crash reporting with trace log dumps directly on Amazon S3.',
      createdAt: '2026-06-19',
      submissionsCount: 8,
      status: 'active',
      fields: [
        { id: 'bug-1', type: 'text', label: 'AWS lambda Request ID', required: true },
        { id: 'bug-2', type: 'select', label: 'HTTP Failure Status Code', required: true, options: ['500 Internal Error', '502 Bad Gateway', '403 Forbidden'] },
        { id: 'bug-3', type: 'textarea', label: 'JSON Stacktrace Dump', required: true, placeholder: 'Paste string trace log...' },
      ]
    },
    {
      id: 'template-3',
      title: 'Event RSVP & Ticket Registration',
      description: 'Collect attendance records for technical webinars or AWS local community conferences.',
      createdAt: '2026-06-21',
      submissionsCount: 201,
      status: 'draft',
      fields: [
        { id: 'rsvp-1', type: 'text', label: 'Preferred Nickname', required: true },
        { id: 'rsvp-2', type: 'radio', label: 'Will you attend in-person?', required: true, options: ['Yes, count me in!', 'No, online broadcast only', 'Undecided'] },
      ]
    }
  ];
  ...
  const handleLiveFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    fields.forEach((field) => {
      if (field.type === 'checkbox') {
        ...
        submittedData[field.label] = checkedOptions.length > 0 ? checkedOptions.join(', ') : 'None selected';
      } else {
    ...
    setFormSubmissionSuccessMessage(`Form response recorded successfully! Simulated AWS CloudWatch event triggered. Telemetry is saved in DynamoDB table "formflow_entries" and aggregated into your real-time analytics dashboard.`);
    ...
    <div className="w-full h-full">
      ...
      {activeTab === 'builder' && (
        <div className="grid grid-cols-12 gap-8 items-start">
          ...
          <div className="col-span-12 lg:col-span-3 space-y-5">
            <div className="border border-white/5 bg-zinc-950/40 rounded-2xl p-5 premium-shadow sticky top-4">
              ...
              <div className="flex flex-col gap-2.5">
                {[
                  ...
                  { type: 'select' as const, label: 'Select dropdown', desc: 'Selection from listed options' },
                  { type: 'checkbox' as const, label: 'Checkbox selection', desc: 'Choose multiple items' },
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 rounded-xl border border-white/5 bg-zinc-900/30 hover:bg-white/5 hover:border-white/10 text-left transition-all group cursor-pointer outline-none"
                  >
                    <div className="space-y-0.5">
                      <p className="text-xs font-semibold text-zinc-300 group-hover:text-brand-primary transition-colors">
                        {item.label}
                      </p>
                    ...
                    <Plus className="h-4 w-4 text-zinc-500 group-hover:text-brand-primary transition-transform group-hover:rotate-90" />
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="col-span-12 lg:col-span-6 space-y-6">
            <Card className="p-6 relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-brand-primary to-brand-accent" />
              
              {/* TOP ACTION TOOLBAR BUTTONS */}
              <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-5 flex-wrap gap-2 text-zinc-400">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold text-zinc-300 hover:text-white bg-white/5 rounded-md border border-white/5 hover:bg-white/10 transition-all cursor-pointer outline-none"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 text-[11px] font-bold text-zinc-400 hover:text-rose-400 bg-white/5 rounded-md border border-white/5 hover:bg-rose-500/10 hover:border-rose-500/20 transition-all cursor-pointer outline-none"
                  >
              ...
              <div className="space-y-2 border-b border-white/5 pb-4.5 mb-6">
                <input
                  ...
                  className="w-full bg-transparent font-display font-bold text-xl sm:text-2xl text-white outline-none focus:border-b focus:border-white/20 pb-1"
                  placeholder="Form Title"
                />
                <textarea
                  value={formDescription}
                  onChange={(e) => setFormDescription(e.target.value)}
                  className="w-full bg-transparent text-sm text-zinc-400 outline-none resize-none min-h-[40px] focus:border-b focus:border-white/20"
                ...
                <AnimatePresence initial={false}>
                  {fields.length === 0 ? (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="text-center py-12 rounded-xl border border-dashed border-white/10"
                    >
                      <p className="text-zinc-500 text-sm">Your form has no fields yet.</p>
                      <p className="text-zinc-600 text-xs mt-1">Select field tools on the left menu to build layout.</p>
                    </motion.div>
                  ) : (
                      ...
                      return (
                        <motion.div
                          ...
                          className={cn(
                            ...
                            dragOverIndex === idx ? "border-dashed border-indigo-500 bg-indigo-500/10" : ""
                          )}
                          onClick={() => setSelectedFieldId(field.id)}
                        >
                          {/* Row Controls */}
                          <div className="absolute right-3.5 top-3.5 flex items-center gap-1.5 opacity-0 group-hover/row/row:opacity-100 transition-opacity">
                            <Tooltip content="Config options">
                              <button
                                ...
                                className="p-1 rounded bg-zinc-950 text-zinc-400 hover:text-white border border-white/5 cursor-pointer"
                              >
                            ...
                            <Tooltip content="Delete row">
                              <button
                                ...
                                className="p-1 rounded bg-zinc-950 text-rose-400 hover:text-rose-300 border border-white/5 cursor-pointer"
                              >
                                <Trash2 className="h-3.5 w-3.5" />
                              </button>
                            </Tooltip>
                          ...
                          <div className="absolute left-3 top-3.5 flex items-center gap-1.5 z-10 select-none">
                            <div 
                              className="text-zinc-600 hover:text-indigo-400 p-0.5 cursor-grab active:cursor-grabbing"
                            ...
                            <div className="flex flex-col gap-0.5">
                              <button
                                ...
                                disabled={idx === 0}
                                title="Move Field Up"
                                className="p-0.5 rounded text-zinc-600 hover:text-indigo-400 disabled:opacity-25 disabled:hover:text-zinc-600 cursor-pointer transition-colors"
                              >
                            ...
                            <span className="text-[10px] font-bold text-indigo-400/80 font-mono ml-1">
                              0{idx + 1}
                            </span>
                          </div>

                          <div className="pt-2 w-full">
                            <p className="text-xs font-semibold text-zinc-300 mb-1.5 flex items-center gap-1">
                            ...
                            {field.type === 'text' && (
                              <div className="h-9 px-3.5 bg-zinc-900/40 rounded-lg border border-white/5 text-zinc-500 text-xs flex items-center">
                            ...
                            {field.type === 'textarea' && (
                              <div className="h-16 p-3 bg-zinc-900/40 rounded-lg border border-white/5 text-zinc-500 text-xs">
                                {field.placeholder || 'Long response lines...'}
                              </div>
                            )}

                            {field.type === 'select' && (
                              <div className="h-9 px-3.5 bg-zinc-900/40 rounded-lg border border-white/5 text-zinc-400 text-xs flex items-center justify-between">
                              ...
                              <div className="flex gap-4 flex-wrap pt-1.5">
                                {(field.options || ['Check Option 1']).map((opt, i) => (
                                  <div key={i} className="flex items-center gap-2">
                                    <div className="h-4.5 w-4.5 rounded border border-white/10 bg-zinc-950/40" />
                            ...
                            {field.type === 'radio' && (
                              <div className="flex gap-4 flex-wrap pt-1.5">
                                {(field.options || ['Radio 1']).map((opt, i) => (
                                  <div key={i} className="flex items-center gap-2">
                                    <div className="h-4.5 w-4.5 rounded-full border border-white/10 bg-zinc-950/40" />
                                    <span className="text-xs text-zinc-400">{opt}</span>
                                  </div>
                                ))}
                              </div>
              ...
              <div className="border-t border-white/5 mt-8 pt-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <Button
                    ...
                    disabled={fields.length === 0}
                    className="bg-indigo-600 hover:bg-indigo-500 h-9 px-4.5 text-xs w-full sm:w-auto"
                  >
            ...
            <div className="border border-white/5 bg-zinc-950/40 rounded-2xl p-5 premium-shadow sticky top-4">
              ...
              {selectedFieldId ? (
                (() => {
                  ...
                  if (!activeField) return <p className="text-xs text-zinc-500">No field matches selection</p>;
                    ...
                    <div className="space-y-4 text-xs">
                      ...
                      {['select', 'checkbox', 'radio'].includes(activeField.type) && (
                        <div className="space-y-2">
                          ...
                          <textarea
                            ...
                            className="w-full min-h-[70px] p-2 bg-zinc-950/50 rounded-lg border border-white/5 text-zinc-200 font-mono text-xs focus:ring-1 focus:ring-brand-primary"
                          />
                        </div>
                      )}

                      <div className="flex items-center justify-between p-3.5 bg-white/[0.02] border border-white/5 rounded-xl">
                        <span className="font-semibold text-zinc-300">Mandatory / Required</span>
                        <input
                          type="checkbox"
                      ...
                      <div className="border-t border-white/5 pt-4">
                        <Button
                          ...
                          className="w-full text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 h-8"
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                          Delete Row Element
                        </Button>
                ...
                <div className="text-center py-8">
                  <p className="text-xs text-zinc-500">Pick any element to access field properties configuration.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="max-w-xl">
            ...
            <p className="text-xs text-zinc-400 mt-1">
              Bootstrap cloud forms immediately using optimized template parameters. Choosing any template will replace elements in your current builder console workspace.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {templates.map((tpl) => (
              <Card key={tpl.id} className="relative overflow-hidden flex flex-col p-6 h-full justify-between">
                ...
                <CardContent className="p-0 py-4">
                  <div className="bg-zinc-950/60 rounded-xl p-3 border border-white/5 space-y-1.5 text-[11px] font-mono">
                    <p className="text-brand-accent tracking-widest uppercase text-[9px] font-bold">fields preview:</p>
                    {tpl.fields.map((f, i) => (
                ...
                <div className="pt-2 border-t border-white/5 flex items-center justify-between">
                  <span className="text-[10px] text-zinc-500">Hits: ~{tpl.submissionsCount}</span>
                  <Button
                    variant="secondary"
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: 'Total Form Hits', val: totalViews, suffix: 'views', tag: 'Traffic', icon: Eye, color: 'text-indigo-400 bg-indigo-500/10' },
              { title: 'Valid Ingests', val: totalSubmissions, suffix: 'records', tag: 'DynamoDB', icon: FileCheck, color: 'text-emerald-400 bg-emerald-500/10' },
              { title: 'Ingestion conversion', val: `${conversionRate}%`, suffix: 'completed', tag: 'Efficiency', icon: TrendingUp, color: 'text-amber-400 bg-amber-500/10' },
              { title: 'Avg Ingestion Duration', val: '42s', suffix: 'response time', tag: 'Performance', icon: Clock, color: 'text-violet-400 bg-violet-500/10' },
            ].map((card, i) => {
              ...
              return (
                <Card key={i} className="p-5 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-1.5 mt-4 border-t border-white/5 pt-3">
                    ...
                    <span className="text-[10px] text-zinc-400 font-medium">Auto-computed by AWS</span>
                  </div>
                </Card>
              );
            })}
          ...
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-12 lg:col-span-8">
              <Card className="p-6">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-400 mb-6 font-display">
                  Schema Ingestion Record Volume (Daily)
                </h3>
                <div className="h-[280px] w-full font-sans">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={submissionsTimeData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
              <div>
                ...
                <p className="text-[10px] text-zinc-500 mt-0.5">
                  Click any ingestion request below to expand the full key-value payload mapped directly to your active form fields.
                </p>
              </div>
              <Badge variant="success" className="self-start sm:self-center">DynamoDB Connected</Badge>
            </div>
                    ...
                    return (
                      <React.Fragment key={row.id}>
                        <tr 
                          ...
                          className="hover:bg-white/[0.02] cursor-pointer transition-colors border-b border-white/[0.02]"
                        >
                          <td className="py-3.5 px-4 font-mono text-[11px] text-indigo-400 font-bold flex items-center gap-2">
                            {isExpanded ? (
                          ...
                          <td className="py-3.5 px-4">
                            <Badge variant="secondary" className="bg-white/5 text-zinc-400 text-[10px] py-0.5 px-2 border border-white/5">{row.browser}</Badge>
                          </td>
                          <td className="py-3.5 px-4 text-right text-zinc-500 font-mono text-[10px]">{row.time}</td>
                        </tr>
                        {isExpanded && (
                          <tr>
                            <td colSpan={4} className="bg-zinc-950/80 px-8 py-5 text-xs rounded-lg">
                              <div className="space-y-4 max-w-2xl">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-[9px] text-zinc-500 font-mono bg-white/5 px-2 py-0.5 rounded">UUID SECURED</span>
                                </div>
                                {row.submittedData && Object.keys(row.submittedData).length > 0 ? (
                                  <div className="grid grid-cols-1 gap-2 border border-white/5 bg-[#09090b]/60 p-4 rounded-xl">
                                    {Object.entries(row.submittedData).map(([lbl, val]) => (
                                      <div key={lbl} className="flex flex-col sm:flex-row sm:items-start sm:justify-between py-1.5 border-b border-white/[0.02] last:border-0 gap-1">
                                        <span className="text-zinc-400 font-semibold">{lbl}</span>
                                        <span className="text-zinc-100 font-bold bg-white/5 px-2.5 py-1 rounded text-left sm:text-right max-w-md truncate font-sans text-[11px]">
                                          {val}
                                        </span>
                                      </div>
                                    ))}
                                  ...
                                  <div className="p-3 border border-dashed border-white/10 rounded-xl bg-white/[0.01] text-zinc-500 italic text-center">
                                    No custom responses configured on this default dummy entry log. Try testing your live built form in the "Form Builder" to submit custom answers!
                                  </div>
                                )}
                              </div>
                            </td>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 h-10 px-3.5 bg-zinc-950/60 border border-white/5 rounded-xl text-zinc-500 text-xs font-mono select-all"
                  />
                  <Button variant="secondary" onClick={() => {
                    navigator.clipboard.writeText("https://api-gateway.us-east-1.amazonaws.com/deploy/v3/submissions");
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    placeholder="*"
                    className="w-full h-10 px-3.5 bg-zinc-950/40 border border-white/10 rounded-xl text-zinc-100 text-xs focus:ring-1 focus:ring-brand-primary outline-none"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">
                  ...
                  <select className="w-full h-10 px-3.5 bg-zinc-950/40 border border-white/10 rounded-xl text-zinc-400 text-xs outline-none">
              ...
              <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl space-y-2">
                <p className="text-xs font-semibold text-zinc-300">Form Submission webhook triggers</p>
                <p className="text-xs text-zinc-500 leading-normal">
                  Send incoming responses to subnetwork automation platforms like Slack or Zapier immediately when saved in DynamoDB.
                </p>
                <input
                  type="text"
              ...
              <div className="pt-4 border-t border-white/5 flex justify-end gap-3">
                <Button variant="ghost" onClick={() => showToast("Settings configuration bypassed", "info")}>
                  Discard
                </Button>
                <Button variant="primary" onClick={() => showToast("Simulated sandbox configurations recorded.", "success")}>
      ...
      >
        <div className="space-y-6 text-zinc-300 text-sm">
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-zinc-950/40 p-4 rounded-xl border border-white/5">
            <div className="space-y-0.5 text-left">
              <p className="text-xs font-semibold text-zinc-200">Interactive Form Simulation Engine</p>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mr-1">Viewport:</span>
                <div className="flex items-center bg-zinc-950/80 p-1 rounded-lg border border-white/5 gap-1">
                  {[
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold transition-all cursor-pointer outline-none border",
                          isActive 
                            ? "bg-indigo-500/20 text-indigo-400 border-indigo-500/30 shadow-[0_2px_8px_rgba(99,102,241,0.15)]"
                            : "text-zinc-500 hover:text-zinc-350 hover:bg-white/[0.02] border-transparent"
                        )}
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mr-1">Brand Theme:</span>
                <div className="flex items-center gap-1.5 bg-zinc-950/80 p-1 rounded-lg border border-white/5">
                  {[
                    ...
                    <button
                      ...
                      className={cn(
                        "h-4 w-4 rounded-full transition-all duration-150 cursor-pointer border hover:scale-110 outline-none",
                        previewTheme === theme.id ? "scale-110 border-white ring-1 ring-white/10" : "border-transparent opacity-60"
                      )}
                    />
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start text-left">
            ...
            >
              ...
              {previewBreakpoint !== 'desktop' && (
                <div 
                  className="flex items-center justify-between px-3.5 py-1.5 bg-zinc-900 border border-white/5 rounded-lg text-[10px] font-mono text-zinc-400 w-full mx-auto transition-all duration-300"
                  style={{
                    maxWidth: 
                ...
                >
                  <span className="flex items-center gap-1.5 font-sans font-medium text-zinc-300">
                    {previewBreakpoint === 'mobile' && <Smartphone className="h-3.5 w-3.5 text-indigo-400" />}
                    {previewBreakpoint === 'tablet' && <Tablet className="h-3.5 w-3.5 text-emerald-400" />}
                    <span className="capitalize">{previewBreakpoint} Viewport</span>
                  </span>
                  <span className="text-zinc-500 font-bold">{previewBreakpoint === 'mobile' ? '375px × 667px' : '640px × 800px'}</span>
                </div>
              )}
              ...
              >
                <Card className="bg-zinc-950/30 p-6 md:p-7 border border-white/10 relative overflow-hidden w-full transition-all duration-300">
                <div 
                ...
                <div className="space-y-1.5 border-b border-white/5 pb-4.5 mb-6">
                  <h2 className="text-base font-bold text-white tracking-snug flex items-center gap-2">
                ...
                {formSubmissionSuccessMessage ? (
                  <motion.div
                    ...
                    className="p-6 rounded-xl border border-emerald-500/20 bg-emerald-500/10 text-emerald-400 space-y-3 flex flex-col items-center text-center"
                  >
                    ...
                    <p className="text-[11px] text-zinc-400 leading-normal max-w-sm">
                      Your response has been written directly into the simulated DynamoDB database schema and recorded in AWS Analytics telemetry charts!
                    </p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleLiveFormSubmit} className="space-y-5">
                    {fields.length === 0 ? (
                      <div className="py-8 text-center text-zinc-500 text-xs italic border border-dashed border-white/5 bg-zinc-950/20 rounded-xl space-y-2">
                        <p>No active form fields customized yet.</p>
                        <p className="text-[10px] text-zinc-600">Close this dialog and click the field palette on the left panel to append inputs!</p>
                      </div>
                    ) : (
                      fields.map((field) => {
                          ...
                          <div key={field.id} className="space-y-1 text-xs text-left">
                            ...
                            {field.type === 'select' && (
                              <Select
                                ...
                                error={errorMsg}
                              >
                                <option value="" className="bg-zinc-900 text-zinc-500">Pick Option</option>
                            ...
                            {field.type === 'checkbox' && (
                              <div className="space-y-1.5 py-1">
                                <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400 select-none block">
                                  {labelWithReq}
                                </span>
                                <div className="flex flex-col gap-2 pt-1 border border-white/5 bg-zinc-900/40 p-3 rounded-xl">
                                  {field.options?.map((opt, i) => (
                                    <Checkbox
                                      key={i}
                                      label={opt}
                                ...
                                {errorMsg && (
                                  <span className="text-[11px] font-medium text-rose-400 block mt-1">{errorMsg}</span>
                                )}
                              </div>
                            )}
                    ...
                    {fields.length > 0 && (
                      <div className="pt-4.5 border-t border-white/5 flex justify-end gap-3">
                        ...
                        <Button variant="primary" type="submit" className="text-white text-xs h-9 font-bold px-4">
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="border border-white/10 bg-zinc-950/70 rounded-2xl p-5 premium-shadow space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-3">
                  <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Database className="h-3.5 w-3.5" />
                    AWS Gateway REST Payload
                  </span>
                  <Badge variant="success" className="bg-[#10b981]/10 text-emerald-400 border border-emerald-400/20 text-[9px] font-bold px-1.5">
                    Streaming Active
                  </Badge>
                </div>

                <p className="text-[11px] text-zinc-400 leading-normal">
                  Our sandbox models exact serverless telemetry format. The interactive JSON view below serializes client responses dynamically on every typing input:
                </p>

                <div className="relative">
                  <div className="absolute top-2 right-2 text-[9px] font-mono text-zinc-500 font-bold tracking-widest uppercase bg-white/5 px-2 py-0.5 rounded">
                    POST: SUBMIT
                  </div>
                  <pre className="p-4 rounded-xl border border-white/5 bg-zinc-950 text-[11px] font-mono overflow-auto max-h-[290px] text-emerald-400 leading-relaxed scrollbar-thin text-left">
                    {JSON.stringify(
                      {
                        metadata: {
                        ...
                        schema_reference_id: formTitle.toLowerCase().replace(/[^a-z0-9]/g, '_') + "_form",
                        form_responses: (() => {
                ...
                <div className="grid grid-cols-2 gap-2 text-center text-zinc-400">
                  <div className="bg-white/5 p-2 rounded-xl border border-white/5">
                    <span className="block text-[9px] uppercase tracking-wider text-zinc-500 font-bold font-display">Ingestable Keys</span>
                    <span className="text-sm font-semibold text-zinc-100 font-mono">
                      {fields.length} Object Field{fields.length === 1 ? '' : 's'}
                    </span>
                  </div>
                  <div className="bg-white/5 p-2 rounded-xl border border-white/5">
                    <span className="block text-[9px] uppercase tracking-wider text-zinc-500 font-bold font-display">Client Validation</span>
                    <span className={cn(
          ...
          <div className="pt-4 border-t border-white/5 flex justify-end gap-2">
            <Button variant="primary" className="bg-zinc-800 hover:bg-zinc-700 text-xs px-4" onClick={() => setIsPreviewModalOpen(false)}>
      ...
      >
        <div className="space-y-5 text-left text-zinc-300">
          <p className="text-xs text-zinc-400 leading-normal">
            Every form created in FormFlow compiles into a secure, serverless cloud schema configuration. You can host this compiled schema as a direct JSON endpoint, paste it into your serverless AWS SAM template, or route submission events directly to API Gateway.
          </p>
          ...
          <div className="space-y-2">
              ...
              <Button
                ...
                onClick={() => {
                  const compiled = {
                    ...
                    version: "2026-06-22",
                    gateway_routing: "https://api-gateway.us-east-1.amazonaws.com/deploy/v3/submissions",
            ...
            <div className="relative">
              <pre className="p-4 rounded-xl border border-white/5 bg-zinc-950/90 text-[11px] font-mono overflow-auto max-h-[280px] text-emerald-400/90 leading-relaxed scrollbar-thin">
                {JSON.stringify({
                  form_id: formTitle.toLowerCase().replace(/[^a-z0-9]/g, '_') + "_form",
                  title: formTitle,
                  description: formDescription,
          ...
          <div className="bg-indigo-500/5 p-4 rounded-xl border border-indigo-500/10 space-y-1.5">
            <p className="text-xs font-semibold text-zinc-200">Where does form data go upon click?</p>
            <p className="text-[11.5px] text-zinc-400 leading-relaxed">
              When a user submits, their answers are packaged as a JSON document and POSTed to the <strong className="text-zinc-200">gateway_routing</strong> endpoint. An AWS Lambda trigger automatically maps these structured answers as document attributes inside Amazon DynamoDB, safely storing them without needing self-managed servers.
            </p>
          </div>

          <div className="pt-4 border-t border-white/5 flex justify-end">
            <Button variant="primary" className="bg-indigo-600 hover:bg-indigo-500 text-xs px-4 py-2" onClick={() => setIsSchemaExportModalOpen(false)}>
              Got it, Close
            </Button>
          </div>
        </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4.5 py-3.5 rounded-2xl border border-white/10 bg-zinc-950/95 text-zinc-100 shadow-2xl backdrop-blur-xl max-w-sm font-sans"
          >
            <div className={cn(
            ...
            <button 
              ...
              className="ml-3 text-zinc-500 hover:text-zinc-200 text-xs font-bold transition-colors cursor-pointer"
            >
              ✕
            </button>
          </motion.div>
